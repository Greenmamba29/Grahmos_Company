...
including Casius Nexus and the other local executive or specialist agents.
...
6. The run exists, but the reviewing runtime cannot inspect it because the
   Paperclip board session or machine credential is missing.
...
### 3. Check review access before inferring a run failure

From the reviewing runtime, verify that you can inspect the Paperclip issue:
...
If those routes return `401`, the review runtime is missing board
authentication or a supported service credential. Do not guess whether the run
is stale, hung, or progressing quietly until those reads succeed.

### 4. Classify the failure correctly

Use the first missing prerequisite to classify the incident:

- Missing `hermes` binary -> platform or infrastructure blocker
- Missing workspace or repo checkout -> workspace or bootstrap blocker
- Missing managed instructions bundle -> agent configuration blocker
- Missing review access to Paperclip issue routes -> board access blocker
- External system dependency with no execution path -> dependency blocker

Do not leave the issue `in_progress` if the run cannot execute useful work or
if the reviewer cannot inspect enough evidence to choose a watchdog action.

### 5. Leave durable progress
...
- repository documentation or runbook update when the failure mode was unclear.

### 6. Choose the final disposition
...
- Local adapter or container setup -> platform or infrastructure owner
- Missing Paperclip instructions bundle -> Paperclip admin or company operator
- Invalid repo/workspace configuration -> repo or adapter owner
- Missing board auth or machine credential -> Paperclip board/operator or
...
- The existing GrahmOS setup docs already track these known failure modes:
  ...
  - missing Paperclip board authentication for review heartbeats.
