...
echo
echo "[2/19] help surface includes current-issue helpers"
"$API_SCRIPT" --help | rg 'build-resume-comment|build-status-update|build-done-update|build-markdown-document|build-plan-confirmation|issue-comment-resume-current|issue-status-current|issue-done-current|issue-update-current|issue-document-put-current|issue-document-put-markdown-current|issue-document-revisions-current|issue-plan-confirmation-current|issue-plan-from-markdown-current|issue-interaction-current|issue-interactions-current|issue-interaction-accept-current|issue-interaction-respond-current' >/dev/null

echo
echo "[3/19] sample payload catalog is available"
...
echo
echo "[4/19] build-resume-comment emits the expected envelope"
...
echo
echo "[5/19] build-status-update emits the expected envelope"
...
echo
echo "[6/19] build-done-update emits the expected envelope"
...
echo
echo "[7/19] build-markdown-document emits the expected envelope"
...
echo
echo "[8/19] request_confirmation sample matches expected schema"
...
echo
echo "[9/19] build-plan-confirmation emits the expected envelope"
...
echo
echo "[10/19] plan document sample matches expected schema"
...
echo
echo "[11/19] ask_user_questions sample matches expected schema"
...
echo
echo "[12/19] suggest_tasks sample matches expected schema"
...
echo
echo "[13/19] interaction accept sample matches expected schema"
...
echo
echo "[14/19] interaction respond sample matches expected schema"
...
echo
echo "[15/19] plan-confirmation shortcut fails with the expected auth gate"
set +e
...
echo
echo "[16/19] markdown document shortcut fails with the expected auth gate"
set +e
...
echo
echo "[17/19] one-shot markdown plan flow fails with the expected auth gate"
set +e
...
echo
echo "[18/19] issue-status-current fails with the expected auth gate"
set +e
...
echo
echo "[19/19] unauthenticated current-issue commands fail with the expected auth gate"
set +e
