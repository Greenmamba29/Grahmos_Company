...
Examples:
  ...
  ./scripts/paperclip-api.sh issue-blocked \
    ...
    "local Paperclip/OpenCode operator" \
    "Repair the Prometheus adapter host so `timeout 20s opencode models` succeeds" \
    "The current runtime cannot mutate issue state without PAPERCLIP_API_KEY."
